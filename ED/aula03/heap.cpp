...
public:
  ...
  ~Heap();
  void escreve_niveis();
  void escreve(const string& prefixo = "", int i = 0); //prefixo opcional, i é pra começar a escrever a partir de tal posição
  void insere(int p);
  int consulta_maxima();
  int extrai_maxima(); // devolve e exclui
...
atribuição de trasnferência nãint main(){}o são fornecidos pelo compilador. Se
